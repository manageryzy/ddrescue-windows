...
#include "../ddrescue-1.25/arg_parser.h"
#include "../ddrescue-1.25/rational.h"
#include "../ddrescue-1.25/block.h"
#include "../ddrescue-1.25/loggers.h"
#include "../ddrescue-1.25/mapbook.h"
#include "../ddrescue-1.25/non_posix.h"
#include "../ddrescue-1.25/rescuebook.h"
...
namespace {
        ...
        const int o_direct_out, const bool synchronous)
    {
        ...
        const int ides = OpenLowLevel(iname, O_RDONLY | O_BINARY | _O_RANDOM);
        ...
        const int odes = OpenLowLevel(oname, O_CREAT | O_WRONLY | o_direct_out | O_BINARY,
            outmode);
        if (odes < 0)
        {
            show_error("Can't open output file", errno); return 1;
        }
        if (LseekFixed(odes, 0, SEEK_SET))
        {
        ...
        const int cluster, const int hardbs)
    {
        ...
        const int ides = OpenLowLevel(iname, O_RDONLY | O_BINARY);
        if (ides < 0)
        {
            show_error("Can't open input file", errno); return 1;
        }
        const long long insize = LseekFixed(ides, 0, SEEK_END);
        if (insize < 0)
        {
        ...
        const int odes = OpenLowLevel(oname, O_RDONLY | O_BINARY);
        if (odes < 0)
        {
            show_error("Can't open output file", errno); return 1;
        }
        ...
        const int ides, const bool ask)
    {
        ...
        if (ask || verbosity >= 1)
        {
            ...
            if (ask || verbosity >= 2)
            {
                ...
                const int odes = OpenLowLevel(oname, O_RDONLY);
                if (odes >= 0)
                {
                    device_id_and_size(LseekFixed(odes, 0, SEEK_END), odes, oid);
                    close(odes);
                }
    ...
    {
        long long insize = LseekFixed(ides, 0, SEEK_END);
        ...
        const bool verify_input_size)
    {
        ...
        const int ides = OpenLowLevel(iname, O_RDONLY | rb_opts.o_direct_in | O_BINARY | _O_RANDOM);
        if (ides < 0)
        {
            show_error("Can't open input file", errno); return 1;
        }
        ...
        const int odes = OpenLowLevel(oname, O_CREAT | O_WRONLY | o_direct_out |
            o_trunc | O_BINARY | _O_RANDOM, outmode);
        if (odes < 0)
        {
            show_error("Can't open output file", errno); return 1;
        }
        ...
        if (preallocate && LseekFixed(odes, 0, SEEK_END) - rescuebook.offset() <
...
#include "../ddrescue-1.25/main_common.cc"
...
bool Rescuebook::reopen_infile()
{
    ...
    ides_ = OpenLowLevel(iname_, O_RDONLY | o_direct_in | O_BINARY | _O_RANDOM);
    if (ides_ < 0)
    {
        final_msg("Can't reopen input file", errno); return false;
    }
    const long long insize = LseekFixed(ides_, 0, SEEK_END);
    if (insize < 0)
    {
...
// NOTE: Unicode is not supported for I do not want to change code in other File -- by mana
int main(const int argc, const char* const argv[])
{
    ...
    enum {
        opt_ask = 256, opt_cm, opt_cpa, opt_ds, opt_eoe, opt_eve, opt_mi,
